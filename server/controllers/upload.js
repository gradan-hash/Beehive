...
const getalluploads = async (req, res) => {
  try {
    const getallhives = await uploads
      .find({})
  ...
  try {
    ...
    if (!getsinglehive) {
      return res
        ...
        .json({ msg: `Item with the id:${hiveId} cannot be found` });
    }

    return res.status(StatusCodes.OK).json({ getsinglehive });
  } catch (error) {
...
const updateupload = async (req, res) => {
  try {
    ...
    const updatehive = await uploads.findOneAndUpdate(
      { _id: hiveId },
      req.body,
      {
        new: true,
    ...
    if (!updatehive) {
      res
        .status(StatusCodes.NOT_FOUND)
        .json({ msg: `Hive with id:${hiveId} cannot be found` });
    }
...
const deleteupload = async (req, res) => {
  try {
    ...
    if (!deletehive) {
      res
        .status(StatusCodes.NOT_FOUND)
        .json({ msg: `hive with the id:${hiveId} cannot be found` });
    }
